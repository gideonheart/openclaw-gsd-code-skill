...
- [ ] **HOOK-02**: Stop hook captures pane content and sends structured wake message to correct OpenClaw agent via session ID
- [ ] **HOOK-03**: All hook scripts exit cleanly (<5ms) for non-managed sessions (no $TMUX or no registry match)
...
- [ ] **HOOK-06**: Stop hook extracts context pressure percentage from statusline with configurable threshold
- [ ] **HOOK-07**: Notification hook (idle_prompt) notifies OpenClaw when Claude waits for user input
- [ ] **HOOK-08**: Notification hook (permission_prompt) notifies OpenClaw on permission dialogs (future-proofing)
- [ ] **HOOK-09**: SessionEnd hook notifies OpenClaw immediately when session terminates
- [ ] **HOOK-10**: PreCompact hook captures state before context compaction
- [ ] **HOOK-11**: Hook scripts support hybrid mode — async by default, bidirectional per-agent via hook_settings.hook_mode

### Wake Message

- [ ] **WAKE-01**: Wake message uses structured sections with clear headers ([PANE CONTENT], [CONTEXT PRESSURE], [AVAILABLE ACTIONS])
- [ ] **WAKE-02**: Wake message includes session identity (agent_id and tmux_session_name)
- [ ] **WAKE-03**: Wake message includes state hint based on pattern matching (menu, idle, permission_prompt, error)
- [ ] **WAKE-04**: Wake message includes trigger type (response_complete vs session_start)
- [ ] **WAKE-05**: Wake message always sent regardless of detected state — OpenClaw agent decides relevance
- [ ] **WAKE-06**: Wake message includes context pressure as percentage + warning level (e.g., `72% [WARNING]`)
...
- [ ] **SPAWN-05**: spawn.sh uses jq for all registry operations (no Python dependency)
...
- [ ] **CONFIG-01**: recovery-registry.json supports system_prompt field (top-level per agent) and hook_settings nested object
- [ ] **CONFIG-02**: recovery-registry.example.json documents system_prompt, hook_settings with realistic multi-agent setup (Gideon, Warden, Forge)
- [ ] **CONFIG-03**: settings.json has all hooks registered (Stop, Notification, SessionEnd, PreCompact), gsd-session-hook.sh removed from SessionStart
- [ ] **CONFIG-04**: Global hook_settings at registry root level with per-agent override (three-tier fallback: per-agent > global > hardcoded, per-field merge)
- [ ] **CONFIG-05**: Default system prompt stored in config/default-system-prompt.txt (tracked in git, minimal GSD workflow guidance)
- [ ] **CONFIG-06**: hook_settings supports strict known fields: pane_capture_lines, context_pressure_threshold, autocompact_pct, hook_mode
- [ ] **CONFIG-07**: Per-agent system_prompt always appends to default (never replaces)
- [ ] **CONFIG-08**: New agent entries auto-populate hook_settings with defaults
...
- [ ] **DOCS-01**: SKILL.md updated with new hook architecture (all hook scripts, hybrid mode, hook_settings)
- [ ] **DOCS-02**: README.md updated with registry schema (system_prompt, hook_settings) and recovery flow
