...
must_haves:
  truths:
    ...
    - "notification-idle-hook.sh bidirectional branch writes JSONL via write_hook_event_record() with outcome=sync_delivered"
    - "notification-permission-hook.sh bidirectional branch writes JSONL via write_hook_event_record() with outcome=sync_delivered"
  ...
  key_links:
    ...
    - from: "scripts/notification-idle-hook.sh"
      ...
      via: "deliver_async_with_logging call (async) and write_hook_event_record call (bidirectional)"
    ...
    - from: "scripts/session-end-hook.sh"
      to: "logs/{SESSION_NAME}.jsonl"
      via: "deliver_async_with_logging call (async only)"
...
Purpose: Validate the JSONL migration pattern on the simplest hooks first — all three have straightforward async delivery paths plus bidirectional branches (notification hooks) or async-only delivery (session-end). This establishes the migration template for the remaining hooks.
...
Run: `grep -c 'JSONL_FILE' scripts/notification-idle-hook.sh` returns 3 (assignment + deliver_async call + write_hook_event_record call in bidirectional branch).
