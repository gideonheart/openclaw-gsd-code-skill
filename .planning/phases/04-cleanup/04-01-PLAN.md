...
<task type="auto">
  ...
  <files>SKILL.md, PRD.md, .planning/PROJECT.md, .planning/ROADMAP.md</files>
  <action>
...
4. **ROADMAP.md** — Update Phase 4 success criteria to match user decisions:
   a. Criterion 4: Change `All existing hook-watcher processes killed via pkill` to `Old hook-watcher processes left to die naturally when sessions end or on reboot (per user decision — no pkill)`
   b. Criterion 5: Change `Watcher state files removed from /tmp` to `Watcher state files in /tmp left to disappear naturally on reboot (per user decision — no manual cleanup)`
...
5. ROADMAP.md Phase 4 success criteria 4 and 5 reflect user decisions (no pkill, no /tmp cleanup)
  </verify>
