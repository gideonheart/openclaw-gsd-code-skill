...
import { spawnDetachedDeferredTyping } from './tui-common.mjs';
import { appendJsonlEntry } from './logger.mjs';

/**
 * Check whether the session is currently in an AskUserQuestion answer flow.
 *
 * Returns true when a pending-answer-{session}.json file exists. This file is
 * created by tui-driver-ask.mjs BEFORE typing keystrokes into the AskUserQuestion
 * TUI, and deleted by the PostToolUse handler after verification. Its presence
 * means the current UserPromptSubmit was triggered by TUI navigation keystrokes
 * (Down/Enter/Space/text) — NOT by a human manually taking over the session.
 *
 * Used by the UserPromptSubmit handler to skip queue cancellation when the user
 * is answering an AskUserQuestion question (not doing a real manual takeover).
 *
 * @param {string} sessionName - tmux session name.
 * @returns {boolean} true if a pending answer file exists for this session.
 */
export function isSessionInAskUserQuestionFlow(sessionName) {
  return existsSync(resolvePendingAnswerFilePath(sessionName));
}
...
/**
 * Read the queue file for a session and find the currently active command.
 *
 * Private helper that deduplicates the read-parse-find-active pattern shared by
 * isPromptFromTuiDriver, writeSubmissionConfirmation, and the bin script's
 * readActiveCommandFromQueueFile.
 *
 * @param {string} sessionName - tmux session name.
 * @returns {{ queueData: Object, queueFilePath: string, activeCommand: Object }|null}
 *   null if no queue file exists or no active command found.
 */
function readQueueAndFindActiveCommand(sessionName) {
  ...
  return { queueData, queueFilePath, activeCommand };
}

const RESULT_TRUNCATION_LENGTH = 200;

/**
 * Truncate a result string to a readable preview length.
 *
 * @param {string|null} result - The full result text.
 * @returns {string|null} Truncated result with ellipsis, or null if no result.
 */
function truncateResultForSummary(result) {
  if (!result) {
    return null;
  }

  if (result.length <= RESULT_TRUNCATION_LENGTH) {
    return result;
  }

  return result.slice(0, RESULT_TRUNCATION_LENGTH) + '…';
}

/**
 * Build the queue-complete summary payload for agent wake-up.
 *
 * Result fields are truncated — the agent already receives the full
 * last_assistant_message in the event metadata. The summary is a compact
 * record of what ran and whether it succeeded, not a content dump.
 *
...
function buildQueueCompleteSummary(queueData) {
  ...
  return {
    ...
    commands: queueData.commands.map(command => ({
      ...
      result_preview: truncateResultForSummary(command.result),
    })),
  };
}
...
export function processQueueForHook(sessionName, incomingHookName, incomingHookSubtype, lastAssistantMessage) {
  ...
  if (nextPendingCommand) {
    ...
    spawnDetachedDeferredTyping(sessionName, nextPendingCommand.command);
 ...
 *
 * Comparison trims both sides because Claude Code may append trailing whitespace
 * to the submitted prompt.
 *
 * @param {string} sessionName - tmux session name.
...
export function isPromptFromTuiDriver(sessionName, submittedPrompt) {
  const result = readQueueAndFindActiveCommand(sessionName);

  if (!result) {
    return false;
  }

  return submittedPrompt.trim() === result.activeCommand.command.trim();
}

/**
 * Write a submission confirmation timestamp onto the active command in the queue file.
 *
 * Called by the UserPromptSubmit handler when it detects that the submitted prompt
 * came from the TUI driver. This sets `confirmed_at` on the active command so the
 * deferred typer can verify that Enter was accepted by Claude Code (closed-loop).
 *
 * @param {string} sessionName - tmux session name.
 * @returns {boolean} true if confirmation was written, false if no queue or no active command.
 */
export function writeSubmissionConfirmation(sessionName) {
  const result = readQueueAndFindActiveCommand(sessionName);
  ...
  const { queueData, queueFilePath, activeCommand } = result;

  const confirmationTimestamp = new Date().toISOString();
  activeCommand.confirmed_at = confirmationTimestamp;
  if (!activeCommand.delivery) {
    activeCommand.delivery = {
      typed_at: null,
      enter_sent_at: null,
      enter_attempts: 0,
  ...
  activeCommand.delivery.submit_confirmed_at = confirmationTimestamp;
  writeQueueFileAtomically(queueFilePath, queueData);

  appendJsonlEntry({
    level: 'info',
    source: 'writeSubmissionConfirmation',
    message: 'Submission confirmed — confirmed_at written to active command',
    session: sessionName,
