 ...
 * 1. Exact match (after trim) — handles plain commands like /clear and argless /gsd:* commands.
 * 2. Command-name prefix match — handles /gsd:* commands with arguments. Tab autocomplete
 *    causes Claude Code to report only the command name in UserPromptSubmit (e.g. "/gsd:plan-phase")
 *    while the queue stores the full command with args (e.g. "/gsd:plan-phase 18"). The prefix
 *    match checks that the submitted prompt equals the command name portion before the first space.
 *
 * @param {string} sessionName - tmux session name.
...
export function isPromptFromTuiDriver(sessionName, submittedPrompt) {
  ...
  const trimmedCommand = activeCommand.command.trim();

  // Exact match: plain commands (/clear) and argless /gsd:* commands
  if (trimmedPrompt === trimmedCommand) {
    return true;
  }

  // Prefix match: /gsd:* commands with args where Tab autocomplete causes
  // UserPromptSubmit to receive only the command name without the arguments.
  // e.g. queue has "/gsd:plan-phase 18" but prompt is "/gsd:plan-phase"
  if (trimmedCommand.startsWith(trimmedPrompt) && trimmedCommand[trimmedPrompt.length] === ' ') {
    return true;
  }
