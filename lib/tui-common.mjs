...
/**
 * Spawn type-command-deferred.mjs as a fully detached background process.
 *
 * Used by processQueueForHook when advancing the queue after a /clear command.
 * The hook handler runs synchronously — Claude Code blocks while the hook executes,
 * so the new TUI prompt cannot render until the hook returns. Calling
 * typeCommandIntoTmuxSession directly from inside a hook causes Enter to be eaten
 * by the still-initializing TUI (even after waitForClaudeCodeTuiReady, which finds
 * the OLD ❯ from /clear output rather than the real new prompt).
 *
 * The deferred script polls for a FRESH empty ❯ prompt (last non-blank line is
 * exactly ❯) which can only appear after the hook has returned and the TUI has
 * fully initialized.
 *
 * @param {string} tmuxSessionName - The tmux session to type into.
 * @param {string} commandText - The full command text to type.
 */
export function spawnDetachedDeferredTyping(tmuxSessionName, commandText) {
  const deferredScriptPath = resolve(SKILL_ROOT, 'bin', 'type-command-deferred.mjs');

  const childProcess = spawn(
    process.execPath,
    [deferredScriptPath, '--session', tmuxSessionName, '--command', commandText],
    {
      detached: true,
      stdio: 'ignore',
    },
  ...
  appendJsonlEntry({
    level: 'info',
    source: 'spawnDetachedDeferredTyping',
    message: 'Deferred typer spawned — will type command once fresh prompt detected',
    session: tmuxSessionName,
    command: commandText,
