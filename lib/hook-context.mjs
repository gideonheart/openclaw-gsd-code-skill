 ...
 * 2.5. Check pause state — paused sessions log one entry and bail (skip stdin I/O)
 ...
 *
 * Checks tmux session, agent registry, and pause state BEFORE reading stdin.
 * Non-managed sessions bail immediately (no stdin read, no logging).
 * Paused sessions log one info entry and bail (skip stdin I/O).
